...
class EWMA:
    ...
    def update(self, time):
        ...
        self.v = dict([(key, value * np.exp(-self.beta * elapsed_time)) for key, value in self.v.items()])
        self.time = time
    ...
    def sorted_itemvalues(self):
        return sorted(list(self.v.items()), key=itemgetter(1, 0), reverse=True)
    ...
    def __str__(self):
        return str(self.time) + ': ' + ','.join(
            [str(key) + ':' + str(value) for key, value in self.sorted_itemvalues()])
...
if __name__ == "__main__":
    ...
    print(counter)
    print(list(map(str, counter.topK(2))))
    counter.update(4)
    print(counter)
    print(list(map(str, counter.topK(2))))
    counter.add('b', 13, 5)
