...
methods = ['LRU', 'LFU', 'FIFO', 'RR', 'EWMAGRAD', 'LMIN', 'TBGRD']
pretty_methods = ['LRU', 'LFU', 'FIFO', 'RR', 'GRD', 'PGA', 'TBGRD']
cmap = dict(list(zip(methods, pretty_methods)))
all_graphs = ['cycle', 'lollipop', 'grid_2d', 'balanced_tree', 'hypercube', 'expander', 'erdos_renyi', 'regular',
              'watts_strogatz', 'small_world', 'barabasi_albert', 'cicular_ladder', 'star', 'powerlaw_tree', 'geant',
              'abilene', 'dtelekom']
threshold = 1000.0
...
if __name__ == '__main__':
    ...
    for filename in myargs.filenames:
        ...
        print(graph, cache, end=' ')

        F = optimal_stats['F']
        L = optimal_stats['L']
        # offline = optimal_stats['offline']
        ...
        # ecline, = ax.plot(times1, np.array(ecgs) , forms[1])
        # taline, = ax.plot(times2, np.array(tags), forms[2])
        #
        optline, = ax.plot(times1, 100 * np.ones(len(times1)), forms[0])
        ecline, = ax.plot(times1, np.array(ecgs) / F * 100, forms[1])
        taline, = ax.plot(times2, np.array(tags) / F * 100, forms[2])
        # offlineline, = ax.plot(times1, np.ones(len(times1)) * offline / F * 100, forms[3])
        ...
        fig.savefig('figs/'+_id + '.pdf', bbox_inches='tight')
        plt.close(fig)
    ...
    caches = ['LRU', 'LFU', 'FIFO', 'RR', 'GRD', 'PGA1', 'PGA10', 'PGA20', 'TBGRD']
    hatches = ['////', '/', '\\', '\\\\', '-', '--', '+', '', '']
    hatchmap = dict(list(zip(caches, hatches)))
    ecg_list = {}
    for cache in caches:
    ...
    for cache in caches:
        print(newind)
        if myargs.hatch:
            rects += ax.bar(newind, ecg_list[cache], width, color=cmaps.plasma(0.5 + 0.5 * i / len(caches)),
                            hatch=hatchmap[cache], linewidth=2, label=cache)
        else:
            rects += ax.bar(newind, ecg_list[cache], width, color=cmaps.plasma(0.5 + 0.5 * i / len(caches)),
                            linewidth=1, label=cache)
        newind += width
        i += 1
    ...
    if myargs.legend:
        ...
        lgd = ax.legend([rects[i * len(graphs)] for i in range(len(caches))], caches, loc=3,
                        bbox_to_anchor=(0., 1.02, 1., .102), mode='expand', ncol=len(caches), borderaxespad=0.)
        # plt.tight_layout()
